    ...
    for i in range(n):
        for j in range(i):
            if nums[j] < nums[i]:
                ...
                if len(dp[j]) + 1 > len(dp[i]) or \
                (len(dp[j]) + 1 == len(dp[i]) and dp[j][-1] == dp[i][-1] - 1):
                    dp[i] = dp[j] + [i]
...
def match_specific_from_end(text, number):
    ...
    matches = [m for m in re.finditer(re.compile(r'(\d+)[．.|\(]|（\d+）|\d+\\.'), text)]
    if matches:
        specific_match = None
        ...
        if specific_match is None:
            ...
            # 在完整文本中匹配第一个符合模式的数字
            start_match = re.search(r'^(\d+)[．.|\(]|（\d+）|\d+\\.', text).group()
            start_number = int(re.search(r'\d+', start_match).group())  # 提取数字部分
